...
| SESSION_ENCRYPTION_KEY | Первый результат генерации из этого подпункта. В .env.example поле пустое: создайте собственный случайный ключ. При обновлении сохраните прежний ключ. |
...
Приложение по умолчанию слушает `127.0.0.1:8000`; настройка `HOST=127.0.0.1` фиксирует это явно. Caddy обращается к нему локально. `TRUSTED_PROXY_IPS=127.0.0.1,::1` разрешает принимать адрес посетителя только от локального прокси; чужие префиксы в `X-Forwarded-For` игнорируются. Закрывайте внешний доступ к 8000 через UFW и firewall провайдера. Порт PostgreSQL 5432 тоже остаётся закрытым снаружи. Для иной схемы прокси используйте [памятку по безопасности](security.md).
...
Для обновления безопасности от 23.09.2026 **сначала замените копию скрипта первым блоком команд**. В нём устранена загрузка зависимостей с правами root. Проверка конфигурации и копирование `.env` теперь также выполняются от пользователя службы; приложение от root скрипт обновлять откажется. Миграция `010_login_security.sql` применяется автоматически и не удаляет объявления, шаблоны или подключённые аккаунты.

Существующую службу можно усилить параметрами `LimitCORE=0`, `RestrictSUIDSGID=true`, `ProtectKernelTunables=true`, `ProtectKernelModules=true`, `ProtectControlGroups=true`, пустыми `CapabilityBoundingSet=` и `AmbientCapabilities=` из примера в шаге 9.8. После изменения unit выполните `sudo systemd-analyze verify /etc/systemd/system/elonbot.service`, затем `sudo systemctl daemon-reload` и `sudo systemctl restart elonbot`. Скрипт обновления сам unit не переписывает. `LimitCORE=0` отключает дампы процесса, которые могут содержать расшифрованные сессии.
